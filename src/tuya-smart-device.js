const VER = '5.0.1 + ms fixes'; // all fixes start  'ms XX/XX/XX'
...
module.exports = function (RED) {
    function TuyaSmartDeviceNode(config) {
        ...
        // ms 26/08/22: anonymize the log: deleted 1 line
        ...
        if (this.storeAsCreds) {
            ...
            // ms 26/08/22: anonymize the log: changed 1 line
            // console.log('Using secrets :: ', secret);
            node.log('Using credentials for ', this.name);
        ...
        // ms 08/06/21: reduce and anonymize the log: replaced 7 lines
        ...
        this.tuyaVersion =
            ...
            isNaN(config.tuyaVersion)
             ? '3.1'
             : config.tuyaVersion.trim();
        // ms 07/06/21: to send 'DISCONNECTED' message at startup
        ...
        node.on('input', function (msg) {
            ...
            if (['GET', 'SET', 'REFRESH'].indexOf(operation) != -1) {
                ...
                if (!tuyaDevice.isConnected()) {
                    ...
                    setStatusOnError(errText, 'Device not connected !', {
                        context: {
                            message: errText,
                            // ms 08/06/21: reduce and anonymize the log
            ...
            case 'CONTROL':
                if (msg.payload.action == 'CONNECT') {
                    // ms 08/06/21: added check. CONNECT only if disconnected
                    if (!tuyaDevice.isConnected())
                        //
                        startComm();
                } else if (msg.payload.action == 'DISCONNECT') {
                    // ms 08/06/21: added check, DISCONNECT only if connected
                    if (tuyaDevice.isConnected())
                        //
                        closeComm();
                    ...
                    // ms 08/06/21: difference CONNECT vs RECONNECT: RECONNECT forces always, added 2 lines
                    if (tuyaDevice.isConnected)
                        closeComm();
                    //
                    startComm();
                    // ms 26/08/22: added new COMMAND, added 10 lines
...
`Event subscription: shouldSubscribeData=>${shouldSubscribeData} , shouldSubscribeRefreshData=>${shouldSubscribeRefreshData}`
        ...
        const startComm = () => {
            // ms 08/06/21: one line deleted: disconnect  not required here.
            //      closeComm();
            ...
            findTimeoutHandler = setTimeout(() => {
                ...
                // ms 08/06/21: reduce and anonymize the log: added 5 lines, changed 4 lines
            ...
            publicParams
          )} , findTimeout :  ${node.findTimeout} , retryTimeout:  ${node.retryTimeout
          } `);
                //
                //        node.log(
                //          `startComm(): Connecting to Tuya with params ${JSON.stringify(
        ...
        const setStatusOnError = function (
            ...
            data) {
            // ms 08/06/21: line deleted: not required the console output. Deleted one line
            //    node.error(errorText, data);
            //

            // ms 08/06/21: ERROR is a MESSAGE, it doesn't have to change the internal status CONNECTED/DISCONNECTED... modefied some lines
            node.send([null, {
                        payload: {
            ...
            //        node.send([null, sendDeviceConnectStatus()]);
            //      }
            return node.status({         // ms: It is OK to change the node look if ERROR msg
                fill: 'red',
                shape: 'ring',
                text: errorShortText
            });
        ...
        const connectionParams = {
            ...
            // ms 07/06/21: added one line, set REFRESH under user control
        ...
        tuyaDevice.on('error', (error) => {
            ...
            setStatusOnError(error, 'Error : ' + JSON.stringify(error), {
                context: {
                    message: error,
                    // ms 08/06/21: anonymize the log:  added one line, deleted 2
                    device: node.deviceName,
                    //       deviceVirtualId: node.deviceId,
                    deviceIp: node.deviceIp
                    //       deviceKey: node.deviceKey,
        ...
        // ms 26/08/22: deplaced down 1 line
...
`Data from device  [event:dp-refresh]: ${JSON.stringify(data)}`);
                ...
                // ms 26/08/22: deplaced up 1 line
            }
        });
        //  }
        ...
        let connectDevice = () => {
            ...
            if (tuyaDevice.isConnected() === false) {
              ...
              e
                    ...
                    if (shouldTryReconnect) {
                        // ms 08/06/21: reduce the log, deleted two lines
        ...
        let findDevice = () => {
            ...
            .then(() => {
                // ms 08/06/21: reduce the log, deleted one line
            ...
            .catch((e) => {
                ...
                setStatusOnError(e.message, "Can't find device", {
                    context: {
                        message: e,
                        // ms 08/06/21: reduce and anonymize the log, added one deleted  two lines
                        device: node.deviceName,
                        //          deviceVirtualId: node.deviceId,
                        deviceIp: node.deviceIp
                        //            deviceKey: node.deviceKey,
        ...
        // Start probing
        // ms 08/06/21: deplaced here, always
        ...
        if (!node.disableAutoStart) {
            ...
            // ms 08/06/21: deplaced before 4 lines. always status DISCONNECTED at start
            // If we dont use timeout , state will not be emitted,
            //     setTimeout(() => {
            //       setStatusDisconnected();
            //     }, 1000);
